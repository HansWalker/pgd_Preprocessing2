...
def evaluate_checkpoint(filename):
    ...
    for i in range(30):
        ...
        g3=tf.Graph()
        epsilonc=0
        mnist_loop_list=np.zeros([2,int(math.ceil(num_eval_examples / eval_batch_size))])
        mnist_loop_list_corr=np.zeros([2,int(math.ceil(num_eval_examples / eval_batch_size))])
    ...
    #           with tf.device("/cpu:0"):
            ...
            with tf.Session() as sess3:
                ...
                for ibatch in range(num_batches):
                    ...
                    cur_corr_adv, cur_xent_adv = sess3.run(
                        [model3.num_correct,model3.xent],
                        feed_dict = dict_adv)
                    mnist_loop_list[0,ibatch]=epsilonc
                    mnist_loop_list[1,ibatch]=cur_corr_adv/eval_batch_size
                    mnist_loop_list_corr[0,ibatch]=epsilonc
                    mnist_loop_list_corr[1,ibatch]=cur_corr_nat/eval_batch_size
        ...
        np.save('loop_data/mnist/mnist_loop_list'+str(i+15)+".npy",mnist_loop_list)
        np.save('loop_data/mnist/mnist_loop_list_corr'+str(i+15)+".npy",mnist_loop_list_corr)
# Infinite eval loop
while True:

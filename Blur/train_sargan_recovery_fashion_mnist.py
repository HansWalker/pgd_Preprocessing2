...
def get_fashion_mnist_data(train_batch_size, val_batch_size):
    mnist = datasets.FashionMNIST(root='/data', train=True, transform=torchvision.transforms.ToTensor(), target_transform=None, download=True).data.float()
    
    data_transform = Compose([ Resize((224, 224)),ToTensor(), Normalize((mnist.mean()/255,), (mnist.std()/255,))])
    train_loader = DataLoader(datasets.FashionMNIST(root='/data', train=True, transform=data_transform, target_transform=None, download=True),
                              batch_size=train_batch_size, shuffle=True)

    val_loader = DataLoader(datasets.FashionMNIST(root='/data', train=False, transform=data_transform, target_transform=None, download=True),
...
def main(args):
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in progress_bar:
            ...
            sample_test_file_1 = os.path.join(output_path, 'image1.jpg')
            if epoch % SAVE_EVERY_EPOCH == 0:
            ...
            sample_test_file_2 = os.path.join(output_path, 'image2.jpg')
            ...
            sample_test_file_3 = os.path.join(output_path, 'image3.jpg')

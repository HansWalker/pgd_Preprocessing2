...
img_size = (224,224,1)
experiment_name = '/sargan_mnist_gaussian_corrupted'
output_dir = 'outputs'
...
def get_fashion_mnist_data(train_batch_size, val_batch_size):
    #mnist = datasets.MNIST(root='/data', train=True, transform=torchvision.transforms.ToTensor(), target_transform=None).data.float()
    #torch.save(mnist, 'MNIST_data')
    mnist = torch.load('data/MNIST_data')
    data_transform = Compose([ Resize((224, 224)),ToTensor(), Normalize((mnist.mean()/255,), (mnist.std()/255,))])
    #train_data_save=datasets.MNIST(root='/data', train=True, transform=data_transform, target_transform=None)
    #torch.save(train_data_save,'MNIST_train')
    train_data=torch.load('data/MNIST_train')
    train_loader = DataLoader(train_data,
                              batch_size=train_batch_size, shuffle=True)
    #torch.save(train_data_save,'MNIST_val')
    val_data=torch.load('data/MNIST_val')
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in progress_bar:
            ...
            if epoch % SAVE_EVERY_EPOCH == 0: 
                ...
                sample_test_file_1 = os.path.join(output_path, 'image_original_%d.jpg' % image_number)
                ...
                sample_test_file_2 = os.path.join(output_path, 'image_corrupted_%d.jpg' % image_number)
                ...
                sample_test_file_3 = os.path.join(output_path, 'image_recovered_%d.jpg' % image_number)
                image_number+=1
